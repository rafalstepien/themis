...
from src.engine.domain.models import ChangedFile, ChangeType
from tests.unit.engine.domain.factories import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ChangeType.RENAMED,
            "old-path -> new-path",
            id="Displays arrow when file is renamed.",
        ),
        pytest.param(
            ChangeType.MODIFIED,
            "new-path",
            id="Displays new path when file is modified.",
        ),
        pytest.param(
            ChangeType.ADDED,
            "new-path",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ChangedFileFactory.build(too_large=True),
            False,
            id="File marked as too large by GitLab",
        ),
        pytest.param(
